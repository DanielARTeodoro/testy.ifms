create database DB_Escola_Prova_new;

Use DB_Escola_Prova_new;
...
-- 1.	Mostre o nome de todos os alunos que não estão ligados a nenhuma disciplina e que tenham o sobrenome Silva. (1.0)

select aluno.nome
from disciplina, turma_professor_disciplina, turma, turma_aluno, aluno 
where disciplina.codigo = turma_professor_disciplina.codDisciplina 
and turma_professor_disciplina.codTurma = turma.codigo 
and turma.codigo = turma_aluno.codTurma 
and turma_aluno.codAluno = aluno.codigo 
and aluno.codigo not like disciplina.codigo 
group by aluno.nome asc
having aluno.nome like '%Silva%';
...
select aluno.nome from aluno, disciplina
where aluno.codigo;

select aluno.nome, disciplina.nome from disciplina, turma_professor_disciplina, turma, turma_aluno, aluno 
where aluno.codigo = turma_aluno.codAluno
and turma_aluno.codTurma = turma.codigo
and turma_professor_disciplina.codTurma = turma.codigo
and turma_professor_disciplina.codDisciplina = disciplina.codigo
and disciplina.nome = null;

select nome from aluno
where aluno.nome like '%Silva%'
 and aluno.codigo not in(
...
-- 2.	Mostre quais disciplinas têm carga horária acima da média. (3.0)

select disciplina.nome, avg(disciplina.cargahoraria) as Media from disciplina;
select disciplina.nome, disciplina.cargahoraria from disciplina 
where disciplina.cargahoraria > '94';

select disciplina.nome, avg(disciplina.cargahoraria) as 'Media' from disciplina
group by disciplina.nome asc
having disciplina.cargahoraria > avg(disciplina.cargahoraria);

select nome, cargahoraria from disciplina
group by nome, cargahoraria asc
having cargahoraria > (select avg(cargahoraria) from disciplina);

-- 3.	Mostre o código de todos os endereços cadastrados, mas, que não estão associados a ninguém (2.0)

select distinct (endereco.codigo)
from aluno, turma_aluno, turma, turma_professor_disciplina, professor, endereco 
where aluno.codEndereco = endereco.codigo 
and professor.codEndereco = endereco.codigo
and aluno.nome not like endereco.codigo 
and professor.nome not like endereco.codigo;


-- 4.	Mostre o nome de todos os professores que já ministraram mais de uma disciplina (diferente), 
-- ainda na mesma consulta, 
-- retorne também a quantidade de disciplinas diferentes que cada professor ministrou. (3.0)

select professor.nome, count(distinct(professor.nome))
from professor, turma_professor_disciplina, disciplina 
where professor.codigo = turma_professor_disciplina.codProfessor 
and turma_professor_disciplina.codDisciplina =  disciplina.codigo 
group by disciplina.nome;

-- 5.	Comente sobre gerenciamento de transações (1.0)
